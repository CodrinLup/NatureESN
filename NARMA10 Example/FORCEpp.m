function [nrmse,output_tranz,Cfnetwork] = FORCEpp (Nnet, Anetwork, Bnetwork, input_training, output_training, input_testing, output_testing, input_validation, output_validation, rho, mean_output_training, tranz, trans,Ts, Kfnetwork)
...
for i=2:length(input_training)
    ...
    % Next state
    state(:,i)=((Anetwork)*state(:,i-1))+Bnetwork*input_training(:,i)+Kfnetwork*Cfnetwork*state(:,i-1);
...
for i=2:length(input_testing)
    state(:,i)=((Anetwork)*state(:,i-1))+Bnetwork*input_testing(:,i)+Kfnetwork*Cfnetwork*state(:,i-1);
